...
class SpreadBot:
    def __init__(self, exchange, pair, buy_price, sell_price, amount, 
        ...
        Args:
            exchange: CCXT exchange object (Kraken)
            pair: Trading pair (e.g., 'USDG/USD')
            buy_price: Buy at this price (0.9999)
            sell_price: Sell at this price (1.0001)
            amount: Amount to trade in base currency (USDG)
            telegram_token: Telegram bot token
            chat_id: Your Telegram chat ID
            check_interval: How often to check orders (seconds)
        """
    ...
    def run_cycle(self):
        ...
        while not self.check_buy_filled():
        ...
        while not self.check_sell_filled():
            time.sleep(self.check_interval)
    ...
    def run_continuous(self):
        """Run the bot continuously, executing cycles until stopped"""
        cycle_count = 0
        try:
            while True:
                cycle_count += 1
